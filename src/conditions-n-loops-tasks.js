...
function convertToRomanNumerals(num) {
  ...
  if (rem >= 10) {
    for (let i = 0; i < Math.floor(rem / 10); i += 1) {
      result += 'X';
    }
    rem %= 10;
  }
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < size; i += 1) {
    if (leftSum === arrSum - arr[i]) return i;
    leftSum += arr[i];
    arrSum -= arr[i];
  }
  return -1;
...
function sortByAsc(arr) {
  ...
  while (i < resultArr.length) {
    ...
    let j = i;
    while (j > 0 && current < resultArr[j - 1]) {
...
function shuffleChar(str, iterations) {
  ...
  return !repeatIteration
    ? result
    : resultHistory[(iterations % repeatIteration || repeatIteration) - 1];
}
